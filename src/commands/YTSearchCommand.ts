...
`,
    ...
    async handleCommand(args: string[], jid: string, user: string, sock: WebSocketInfo, sessionService: SessionService, msg: proto.IWebMessageInfo): Promise<void> {
        ...
        if (!isNaN(nomor)) {
            ...
            const session = await sessionService.getSession<{ videos: any[] }>(jid, user);
            if (!session || !session.data || !session.data.videos || session.data.videos.length < nomor || nomor < 1) {
                await sock.sendMessage(jid, { text: "Tidak ada hasil pencarian sebelumnya atau nomor tidak valid. Silakan cari dulu dengan ytsearch." });
                return;
            }
            ...
            await sock.sendMessage(jid, { text: `Mengunduh ${isAudio ? "audio" : "video"} dari: ${video.title}` });
            try {
                const { YtDlpWrapper } = await import("../utils/ytdlp.js");
                const ytdl = new YtDlpWrapper();
                const result = isAudio ? await ytdl.downloadAudio(video.url) : await ytdl.downloadVideo(video.url);
                const fileSizeMB = result.buffer.length / (1024 * 1024);
                if (fileSizeMB > 100) {
                    await sock.sendMessage(jid, { text: `❌ File terlalu besar (${fileSizeMB.toFixed(1)}MB). Maksimal 100MB.` });
                    return;
                }
                if (isAudio) {
                    await sock.sendMessage(jid, {
                ...
                await sock.sendMessage(jid, { text: `Gagal mengunduh: ${error?.message || error}` });
            }
            return;
        }
        ...
        try {
            ...
            const videos = result.videos.filter((v) => v.duration.seconds < 1800).slice(0, 5);
