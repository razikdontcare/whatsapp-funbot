...
export class AIConversationService {
  ...
  async addMessage(userId: string, role: AIMessage['role'], content: string, tool_call_id?: string): Promise<void> {
  ...
  private async loadSessionsFromDB(): Promise<void> {
    ...
    try {
      const now = Date.now();
      const activeSessions = await this.conversationCollection.find({ expiresAt: { $gt: new Date(now) } }).toArray();
      ...
      log.info(`Loaded ${activeSessions.length} active AI conversation sessions`);
    } catch (error) {
  ...
  private cleanupExpiredSessions(): void {
    ...
    if (expiredSessions.length > 0) {
      log.info(`Cleaned up ${expiredSessions.length} expired AI conversation sessions`);
    }
  }
}
