...
export class AIResponseService {
    ...
    async getGroupResponses(groupId: string, limit: number = 20): Promise<AIGroupResponse[]> {
        if (!(await this.ensureInitialized())) {
            return [];
        }
        ...
        try {
            ...
            return responses.map((response) => ({
                ...
                expiresAt: response.expiresAt.getTime ? response.expiresAt.getTime() : response.expiresAt,
            }));
        } catch (error) {
