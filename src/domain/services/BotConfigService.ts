...
export class BotConfigService {
    ...
    async getConfig(): Promise<StoredBotConfig> {
        // Check cache first
        if (this.cachedConfig && Date.now() - this.lastCacheUpdate < this.CACHE_TTL) {
     ...
     */
    async updateConfig(updates: Partial<StoredBotConfig>, updatedBy?: string): Promise<boolean> {
        try {
            ...
            await this.collection.updateOne({_id: 'default'}, {$set: updateData}, {upsert: true});
    ...
    async getMergedConfig(): Promise<typeof BotConfig> {
        ...
        return {
            ...
            alternativePrefixes: storedConfig.alternativePrefixes ?? BotConfig.alternativePrefixes,
            allowMentionPrefix: storedConfig.allowMentionPrefix ?? BotConfig.allowMentionPrefix,
            ...
            defaultGameHelp: storedConfig.defaultGameHelp ?? BotConfig.defaultGameHelp,
            unknownCommandResponse: storedConfig.unknownCommandResponse ?? BotConfig.unknownCommandResponse,
            emoji: {
                ...BotConfig.emoji,
                ...storedConfig.emoji,
            },
            ...
            maintenanceMode: storedConfig.maintenanceMode ?? BotConfig.maintenanceMode,
        };
    }
    ...
    async resetToDefaults(updatedBy?: string): Promise<boolean> {
        try {
            ...
            log.info(`Bot configuration reset to defaults by ${updatedBy || 'system'}`);
            return true;
        } catch (error) {
     ...
     * Add user to role
     */
    async addUserToRole(userJid: string, role: UserRole, updatedBy?: string): Promise<boolean> {
        try {
            const roleField = `${role}s` as keyof Pick<StoredBotConfig, 'admins' | 'moderators' | 'vips'>;
            ...
            log.info(`User ${userJid} added to ${role} role by ${updatedBy || 'system'}`);
            return true;
        } catch (error) {
     ...
     */
    async removeUserFromRole(userJid: string, role: UserRole, updatedBy?: string): Promise<boolean> {
        try {
            const roleField = `${role}s` as keyof Pick<StoredBotConfig, 'admins' | 'moderators' | 'vips'>;
            ...
            log.info(`User ${userJid} removed from ${role} role by ${updatedBy || 'system'}`);
            return true;
        } catch (error) {
    ...
    async getConfigHistory(limit: number = 10): Promise<StoredBotConfig[]> {
        try {
            return await this.collection.find({}).sort({lastUpdated: -1}).limit(limit).toArray();
        } catch (error) {
